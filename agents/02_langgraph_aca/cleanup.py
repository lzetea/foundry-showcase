"""Scenario 02 teardown — reverses what ``deploy_and_register.py`` did.

* Deletes the Foundry project connection
  ``contoso-travel-langgraph-aca-endpoint``.
* Rolls the ACA app ``ca-langgraph-agent`` back to the public placeholder
...
def main() -> None:
    ...
    print("\nDone. Run `python -m agents.02_langgraph_aca.deploy_and_register`")
    print("to redeploy.")
